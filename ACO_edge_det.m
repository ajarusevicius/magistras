...
%filename = 'elektroforizebitdepth4';
%filename = 'MzrwFZt';
%filename = 'test48 bit';
filename = '24_bit';
%filename = '32_bit';
...
%image(x);
%%
%img=double(rgb2gray(x))./255;
img=double(rgb2gray(x(1:200,1:200,1:3)))./255;
%img=double(rgb2gray(x(1:200,1:200,1:3)))./65535;
%img=double(rgb2gray(x(1:256,1:256,1:3)))./65535;
%img=double(rgb2gray(x(1:256,1:256,1:3)))./16777215;
figure;
histogram(img);
title('Po normalizavimo ir konvertavimo i pustonius')
%img = double(imread([filename '.jpg']))./255;
%img = double(imread([filename '.jpg']))./16777215;
[nrow, ncol] = size(img);
fprintf('Welcome to demo program of image edge detection using ant colony.\nPlease wait......\n');
%visiblity function initialization, see equation (4)

for nMethod = 1:1;
    %pakeiciau metoda kad butu tik vienas.
    tic
    ...
    for rr =1:nrow
        for cc=1:ncol
            ...
            if size(temp11,1) == 0
                ...
                lambda = 10;
                %Skirtingos funkcijos keiciancios 
                switch nMethod
                    case 1%'Q'
    ...
    % Normalizacija(pikselio sviesumo(intensity) santykis su aplinkiniu
    % sviesumu)
    ...
    %feramono svoris judejime
    alpha = 1;      %equation (4)
    %neuristines info svoris judejime
    beta = 0.1;     %equation (4)
    %atsitiktinio judejimo svoris???
    rho = 0.5;      %equation (11)
    %Feramono mazinimo iteracijos svoris
    phi=0.005;
    %phi = 0.001;     %equation (12), i.e., (9) in IEEE-CIM-06
%%
    %skruzdziu skaicius
...
%%    
    ...
    toc
    %figure;
    %image(uint8(abs((p>=T).*255)));
    figure;
    hold all;
    image(x);  
    %contour(uint8(abs((p>=0.0001).*255)));
    %Pagal vidutini sviesi pjaunamas kalnas
    
    [C,h]=contour(uint8(abs((p>=T).*255)));
    figure 
    [C,h]=contourf(uint8(abs((p>=T).*255)));
    C
    axis ij
    %figure;
    %imcontour(uint8(abs((p>=T).*255)),3);
    imwrite(uint8(abs((p>=T).*255-255)), gray(256), [filename '_edge' num2str(nMethod) '.bmp'], 'bmp');
