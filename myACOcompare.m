function [ans,p,laikas]=myACOcompare(filename)
...
%close all;
% image loading
%filename = 'camera128'; img = double(imread([filename '.bmp']))./255;
...
for nMethod = 1:1;
    ...
    ans=uint8(abs((p>=T).*255));
    %figure(nMethod*2);imagesc(uint8(abs((p>=T).*255-255)));axis equal image;
    %figure(nMethod*2+1);imagesc(uint8(abs((p>=T).*255-255)));axis equal image;
end % end of nMethod
